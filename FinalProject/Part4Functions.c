...
movesList* PosListToMoveList(posList pos_list) {
	...
	char row = pos_temp->pos[0] - pos_temp->next->pos[0];
	char col = pos_temp->pos[1] - pos_temp->next->pos[1];
	pos_temp = pos_temp->next;
	moves_list->head = newMovesCell(row, col);//change to real function later
	moveCell* temp = moves_list->head;


	while (pos_temp->next != NULL) {
		
		row = pos_temp->pos[0] - pos_temp->next->pos[0];
		col = pos_temp->pos[1] - pos_temp->next->pos[1];
		pos_temp = pos_temp->next;
		temp->next = newMovesCell(row, col); 
		temp->next->prev = temp;
		temp = temp->next;
...
posCell* longestPathRec(treeNode* head, int moves_left) {
	...
	treeNodeListCell* current= head->next_possible_positions->node;
